...
# ── 本地改动保护门闸（2026-09-16）──────────────────────────────────────
# 网页端「工具」里的 git 动作会先执行无条件破坏性命令（git_pull 第一步就是
# `git reset --hard`；git_reset_repo_checkout 还会 `git clean -xfd`，连未跟踪
# 文件一起删）。误点一次即静默丢弃 /data/openpilot 下全部本地修改，不可逆。
# 保护①：DisableUpdates=1 时 git_pull 直接拒绝（彻底禁止更新）。
# 保护②：工作区存在未提交改动时，所有破坏性 git 动作一律拒绝。
GUARDED_GIT_ACTIONS = frozenset({
  "git_sync", "git_reset", "git_checkout",
  "git_reset_repo_fetch", "git_reset_repo_checkout",
})

DISABLE_UPDATES_HINT = (
  "更新已被 DisableUpdates 参数禁止，未执行任何 git 操作。\n"
  "如需恢复更新能力，请先把 DisableUpdates 设为 0。"
)


def _updates_disabled() -> bool:
  """DisableUpdates 为真时禁止 git_pull。读不到参数时按未禁止处理。"""
  if not HAS_PARAMS or Params is None:
    return False
  try:
    return bool(Params().get_bool("DisableUpdates"))
  except Exception:
    return False


def _guard_message(action: str, changes: str) -> str:
  lines = [ln for ln in changes.splitlines() if ln.strip()]
  listing = "\n".join(lines[:30])
  more = "" if len(lines) <= 30 else "\n... (共 %d 项)" % len(lines)
  return (
    "检测到 /data/openpilot 存在未提交的本地修改，已拒绝执行 `%s`，以免代码丢失。\n"
    "如需更新，请先提交或备份这些改动。\n\n本地改动：\n%s%s" % (action, listing, more)
  )


async def _git_guard(action: str, repo_dir: str) -> "Optional[str]":
  """返回拒绝原因；返回 None 表示放行。"""
  if action != "git_pull" and action not in GUARDED_GIT_ACTIONS:
    return None
  if action == "git_pull" and _updates_disabled():
    return DISABLE_UPDATES_HINT
  try:
    rc, out = await jobs.capture_exec(["git", "status", "--porcelain"], cwd=repo_dir, timeout=20)
  except Exception:
    return None
  if rc != 0:
    return None
  changes = (out or "").strip()
  if not changes:
    return None
  return _guard_message(action, changes)


def _git_guard_sync(action: str, repo_dir: str) -> "Optional[str]":
  """同步版门闸，供 dispatch_sync 使用。"""
  if action != "git_pull" and action not in GUARDED_GIT_ACTIONS:
    return None
  if action == "git_pull" and _updates_disabled():
    return DISABLE_UPDATES_HINT
  try:
    proc = subprocess.run(["git", "status", "--porcelain"], cwd=repo_dir,
                          capture_output=True, text=True, timeout=20)
  except Exception:
    return None
  if proc.returncode != 0:
    return None
  changes = (proc.stdout or "").strip()
  if not changes:
    return None
  return _guard_message(action, changes)
...
async def run_tool_job(job: Dict[str, Any]) -> None:
  ...
  try:
    ...
    guard_error = await _git_guard(action, repo_dir)
    if guard_error:
      jobs.append(job, guard_error + "\n")
      jobs.finish(job, ok=False, result={"ok": False, "error": guard_error, "error_code": "GIT_GUARD_BLOCKED"}, error=guard_error, error_code="GIT_GUARD_BLOCKED")
      return
...
async def dispatch_sync(request: web.Request, body: Dict[str, Any]) -> web.Response:
  ...
  guard_error = _git_guard_sync(action, "/data/openpilot")
  if guard_error:
    return web.json_response({"ok": False, "error": guard_error, "error_code": "GIT_GUARD_BLOCKED"}, status=409)
