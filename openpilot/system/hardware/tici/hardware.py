...
class Tici(HardwareBase):
  @cached_property
  def amplifier(self):
    # 没有扬声器硬件的机器不要去探放大器：I2C 探测失败会重试 15 次、退避合计 12 秒，
    # 一次开机至少走 3 遍（initialize_hardware 1 遍 + set_power_save 2 遍），白等约 36 秒。
    #   - "mici"：Comma Mic
    #   - "tici"：Comma Three / C3X 早期机型（常见无扬声器）
    #   - 型号里含 "XL" 的：如 three XL（C3XL）
    # 另保留 fork 自带的 HardwareC3xLite 开关（萝卜面板「设备硬件」里可切）。
    device = self.get_device_type()
    if device == "mici" or device == "tici" or "XL" in device or is_c3x_lite():
      return None
    return Amplifier()
