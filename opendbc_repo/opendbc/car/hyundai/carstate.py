...
class CarState(CarStateBase):
  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    #加上ESCC的数据
      ...
      if use_fca:
        # FCA11 can sit on the powertrain bus (CAN 0) or the camera bus (CAN 2).
        # add_if_seen() above registered it on whichever one actually carries it,
        # so pick that same parser here. Reading it from `cp` unconditionally would
        # register the address on CAN 0 through VLDict, which never sees it there,
        # and that in turn fails can_valid.
