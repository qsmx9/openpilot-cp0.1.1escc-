...
def create_acc_commands(packer, enabled, accel, jerk, idx, hud_control, set_speed, stopping, long_override, use_fca, CP, CS, soft_hold_mode, escc):
  ...
  if escc:
    # ESCC keeps the stock radar AEB alive, so SCC12 has to report AEB as enabled.
    # This must be unconditional: on FCA cars use_fca is True, which skips the block
    # above, and an AEB_Status of 0 is read by the cluster (SCC12 AEB_Status receiver
    # list is CLU,ESC) as an AEB fault. sunnypilot upstream sets 2 unconditionally here
    # for the same reason; sending 1 makes the dash flicker between "deactivated" and
    # "active assistance".
    scc12_values["AEB_Status"] = 2  # AEB enabled (stock radar AEB preserved)
