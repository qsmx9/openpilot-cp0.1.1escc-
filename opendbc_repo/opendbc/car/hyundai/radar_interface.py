...
class RadarInterface(RadarInterfaceBase):
  def __init__(self, CP):
    ...
    # ESCC 模式（EnableEscc=1 且车上 0x2AB 出现在 bus0）下必须一并关掉 SCC parser：
    # openpilot 自己会往 bus0 发 SCC11/SCC12（hyundaican.py），此后 bus0 上不再有
    # 车辆侧的 SCC11 RX，只剩 panda 把本机发出的帧回显成 src=128。
    # 继续订阅就会让这个 CANParser 永远 can_valid=False
    #   -> RadarData.errors.canError=True
    #   -> card 里 liveTracks.valid=False
    #   -> radard_dpath 里 radarState.valid=False
    #   -> selfdrived 在屏幕上打出「CAN错误：请检查连接！！」
    ...
    if self.enhanced_scc:
      ...
      # ESCC 点来自原车前向雷达 0x2AB，用枚举默认值 frontRadar：
